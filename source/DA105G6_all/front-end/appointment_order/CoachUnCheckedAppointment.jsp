...
<%@ include file="C_left_bar.file" %>
  ...
  <thead>
    ...
    <tr>
      ...
      <td id='tdstatus${aptmtVO.appointmentOrderNo}'>${appointment_status[aptmtVO.appointmentStatus]}</td>
      <td><button class="btn btn-light" style="padding:15px 25px;" onClick="getAPDetail('${aptmtVO.appointmentOrderNo}')">詳細內容</button></td>
	  <td><input type="button" id='btnprice${aptmtVO.appointmentOrderNo}' onClick="quoteprice('${aptmtVO.appointmentOrderNo}','${aptmtVO.memberNo}')" class="btn btn-light" style="cursor: pointer;" value=" 報價 "></td>
    </tr>	
										</c:if></c:forEach>
										<c:forEach var="aptmtVO" items="<%=list%>" >
    ...
    <tr>
      ...
      <td><button class="btn btn-light" onClick="changeDate({appointmentOrderNo:'${aptmtVO.appointmentOrderNo}',action:'changeclass'})" style="padding:15px 25px;">調課</button></td>
    </tr>											</c:if></c:forEach>											
										<c:forEach var="aptmtVO" items="<%=list%>" >
...
<script>
//quoteprice
function quoteprice(apOrderNo,memberNo){
	Swal.fire({
		  title: '輸入報價',
		  icon:'info',
		  text:'報價後即不可再修改價格',
		  ...
		  if (result.value) {		  
			    $.ajax({
			    	...
			    	data:{
			    		...
			    		'appointmentPrice':result.value
			    	},
			    	success:function(){
			    	var yourNo = "";
			    	var jsonObj = {"message" : "","yourNo":memberNo,"action":"quoteprice"};
			    	  ws.send(JSON.stringify(jsonObj));
					  $('#tdstatus'+apOrderNo).html("確認中");
					  $('#btnprice'+apOrderNo).hide();
					  Swal.fire({
					  title: '報價成功',
					  icon: 'success',
					})
    ...
    $.ajax({
    	...
    	success:function(apo){
    		...
    		if(appointmentPrice==0)
    			appointmentPrice='';    		
			var date = orderTime.substring(0,19);	
				  ...
				  Swal.fire({
					  ...
					  html:`<table class="table"><thead>
						    <tr><th scope="col" nowrap="nowrap">訂單編號`+appointmentOrderNo+`</th>
					  		    <th scope="col" nowrap="nowrap">會員編號`+memberNo+`</th></tr>
						  </thead><tbody>
...
function changeDate(params) {
  ...
  form.method = 'post';
  form.action = '<%=request.getContextPath()%>/appointment_order/appointmentOrder.do';
